    ...
    <div class="row justify-center full-width" style="max-width: 1200px; width: 100%">
      <div class="col-12 col-md-3 q-pr-md q-mb-md">
        <q-card class="q-pa-md text-center shadow-2 bg-white text-dark sidebar-card">
          ...
          <q-btn flat dense color="primary" label="Ver Perfil" class="q-mt-sm" @click="goToProfile" />
        </q-card>
      ...
      <div class="col-12 col-md-6">
        <q-card v-for="palpite in palpites" :key="palpite.id"
          ...
          <div class="image-container">
            <q-img :src="palpite.img_url" class="post-image" spinner-color="primary" fit="contain" @error="onImageError"
              loading="lazy" crossorigin="anonymous" />
          </div>

          <div class="row items-center justify-end q-pl-xl q-gutter-xs">
            <div class="actions-section row q-gutter-sm items-center">
              <ReactionButtons :total-likes="palpite.total_likes || 0" :total-dislikes="palpite.total_dislikes || 0"
                :user-reaction="palpite.user_reaction" :loading="reactingPalpiteId === palpite.id"
              ...
              <q-btn flat round dense icon="content_copy" :color="palpite.link ? 'primary' : 'grey-7'"
                :disable="!palpite.link" size="md" class="action-btn" @click="copiarLink(palpite)">
                <q-tooltip anchor="top middle" self="bottom middle" :offset="[0, 8]">
          ...
          <q-card style="width: 100%; max-width: 600px">
            ...
            <q-card-section style="max-height: 400px; overflow-y: auto">
              <div v-if="selectedPalpite?.comments && selectedPalpite.comments.length" class="q-gutter-sm">
                <div v-for="comment in selectedPalpite.comments" :key="comment.id"
                  class="comment-item q-pa-sm bg-grey-1 rounded-borders">
                  <div class="row items-center q-mb-xs">
                    <q-avatar size="32px" class="q-mr-sm">
                      <img :src="comment.autor_avatar || 'https://i.pravatar.cc/150?u=' + comment.user_id"
                        style="object-fit: cover" />
                    </q-avatar>
                    <div class="text-weight-bold text-primary">{{ comment.autor_nome || 'Usuário' }}</div>
                  </div>
                  <div class="q-mt-xs q-ml-md">{{ comment.texto }}</div>
                  <div class="row items-center q-mt-sm q-ml-md" v-if="comment.total_likes || comment.total_dislikes">
                    <span class="text-caption text-grey-7">
                      <q-icon name="thumb_up" size="14px" /> {{ comment.total_likes || 0 }}
                      <q-icon name="thumb_down" size="14px" class="q-ml-sm" /> {{ comment.total_dislikes || 0 }}
                    </span>
                  </div>
                </div>
              </div>
